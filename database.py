...
def import_from_excel(filepath):
    ...
    # --- 2. Tasksheet(All) — additional tasks with priority ---
    if 'Tasksheet(All)' in wb.sheetnames:
        ws = wb['Tasksheet(All)']
        for row in ws.iter_rows(min_row=3, max_row=ws.max_row, values_only=True):
            task_name = str(row[0] or '').strip() if row[0] else ''
            task_type_raw = str(row[1] or '').strip() if row[1] else ''
            if not task_name or not task_type_raw:
                continue
            # Skip week header rows
            if '–' in task_name[:20] or 'Jun' in task_name[:5] or 'Jul' in task_name[:5]:
                continue

            task_type = normalize_type(task_type_raw)
            task_category = str(row[2] or '').strip()  # Marketing task / Product Task
            status = normalize_status(str(row[3] or ''))
            priority = str(row[4] or '').strip()
            due_date = ''
            if row[5] and isinstance(row[5], datetime):
                due_date = row[5].strftime('%Y-%m-%d')
            notes = str(row[6] or '').strip()
            owner = str(row[7] or '').strip() if len(row) > 7 and row[7] else ''

            task_id = make_task_id(task_name, due_date, owner or 'team')

            # Only insert if not already present from Calendar
            existing = conn.execute("SELECT id FROM tasks WHERE id = ?", (task_id,)).fetchone()
            if not existing:
                source_label = f"Tasksheet ({task_category})" if task_category else "Tasksheet"
                conn.execute("""
                    INSERT OR IGNORE INTO tasks (id, content, type, date, owner, status, approval, comment, link, priority, source, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (task_id, task_name, task_type, due_date, owner, status, 'Draft', notes, '', priority, source_label, now, now))
                if owner:
                    conn.execute("INSERT OR IGNORE INTO owners (name) VALUES (?)", (owner,))
                imported_count += 1

    # --- 3. Individual owner sheets ---
    owner_sheets = {
        'Tasks –\xa0Mahek': 'Mahek',
        'Tasks –\xa0Charu': 'Charu',
        'Tasks - Akshit': 'Akshit',
        'Tasks –\xa0Samriddhi': 'Samriddhi',
        'Tasks –\xa0Aamir': 'Aamir',
        'Tasks-Winston': 'Winston',
    }

    for sheet_name, owner_name in owner_sheets.items():
        ...
        for ridx, row in enumerate(ws.iter_rows(min_row=1, max_row=5, values_only=True), start=1):
            if row[0] and str(row[0]).strip().lower() in ('task', 'date', 'date '):
                header_row = ridx
                break
        if not header_row:
            header_row = 1
        ...
        for row in ws.iter_rows(min_row=header_row + 1, max_row=ws.max_row, values_only=True):
            ...
            if not task_name:
                continue
            # Skip week header rows
            if '–' in task_name[:20] or '-' in task_name[:3] or 'Jun' in task_name[:5] or 'Jul' in task_name[:5]:
                continue
            if task_name.lower() in ('daily', 'last month', 'this week'):
                continue

            task_type = normalize_type(str(row[1] or '')) if len(row) > 1 else ''
            status = normalize_status(str(row[2] or '')) if len(row) > 2 else 'To Do'
            priority = str(row[3] or '').strip() if len(row) > 3 else ''
            due_date = ''
            if len(row) > 4 and row[4] and isinstance(row[4], datetime):
                due_date = row[4].strftime('%Y-%m-%d')
            notes = str(row[5] or '').strip() if len(row) > 5 else ''

            task_id = make_task_id(task_name, due_date, owner_name)
            ...
            if not existing:
                conn.execute("""
                    INSERT OR IGNORE INTO tasks (id, content, type, date, owner, status, approval, comment, link, priority, source, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (task_id, task_name, task_type, due_date, owner_name, status, 'Draft', notes, '', priority, f'Sheet: {owner_name}', now, now))
                conn.execute("INSERT OR IGNORE INTO owners (name) VALUES (?)", (owner_name,))
                imported_count += 1
